...
export class UserDetailsComponent implements OnInit {
  ...
  getRoleData(role) {
    let roleData = this.roles.find(res => res.id === role);
    if (roleData) {
  ...
  addUser() {
    ...
    let userIds = this.userForm.value["userIds"],
      role = this.userForm.value["role"];

    userIds.forEach(function (ele) {
      let user = that.userResponse.find((res) => res.userId === ele.id);
      if (user) {
        let roleFind = that.userRoleObject.findIndex(
          (res) => res.userId === ele.id
        );
        ...
        let recordFind = that.userLists.findIndex(
          (res) => res.userId === ele.id
        );
  ...
  deleteProject(user) {
    let userIndex = this.userLists.findIndex(
  ...
  getUserRole(user) {
    this.userService.getUserRole(this.projectId, user).subscribe((res) => {
      if (res.id) {
        // this.messageService.showMessage({ type: 'success', title: '', body: res.message });
        return res.roleId;
      } else {
        return null;
      }
