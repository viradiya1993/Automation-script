...
import { barChartOption, chartOptions, areaChartColors, areaChartOption, piChartOptions } from '@app/core/helpers/chart.helper';
import { TestBotExecutorService, TestReportService } from '@app/core/services';
...
import { chartBarThickness, chartColors, lineChartColors, piChartColors } from './../../../core/helpers/chart.helper';
import { Progress } from './../../../shared/models/progress.model';
import { TestSuiteResult } from './../../../shared/models/test-suite-result.model';
...
export class DashboardComponent implements OnInit {
  ...
  constructor(private globalService: GlobalService, private testReport: TestReportService,
    public dialog: MatDialog, private testBotExecutorService: TestBotExecutorService) { }
  ...
  private manageEpicReports(epicReport) {
    ...
    console.log(epicReport,'epicReport');
  ...
  private manageStoryReports(storyReport) {
    ...
    console.log(storyReport,'storyReport');
    const passedTagData = [];
    const failedTagData = [];
    const skippedTagData = [];
  ...
  openTestSuiteResultDialog(index: number) {
    this.testBotExecutorService.getConfiguration(this.testSuiteResults[index].testSuiteResultId).subscribe((configuration) => {
      this.testBotExecutorService.getTestSuiteResult(this.testSuiteResults[index].testSuiteResultId).subscribe((testScriptResults) => {
