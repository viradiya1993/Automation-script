...
import { Filter, NameFilterResult, SearchFilter, Story, StoryFilterView, TestScript, TestScriptFilterView, TestScriptView, TestSuiteView } from '@app/shared/models';
...
export class TestScriptService {
  gridApi;
  gridColumnApi;
  testScriptApi: string = Test_Script_API + 'projects/' + localStorage.getItem('projectId') + '/scripts';
  columnDefs: any[] = [
    // {
  ...
  getTestScripts(offset: number) {
    return this.http.get(this.testScriptApi + '?offset=' + offset).pipe(
      map(res => {
  ...
  addTestScript(testScript: TestScript) {
    testScript.organizationId = localStorage.getItem('organizationId');
    testScript.projectId = localStorage.getItem('projectId');
  ...
  removeTestScript(testScriptId: string) {
