...
export class ReleaseService {
  ...
  executeRelease(releaseRun: ReleaseRun) {
    releaseRun.createdBy = JSON.parse(localStorage.getItem('user'));
    return this.http.post(`${this.releaseApi}/${releaseRun.releaseId}/execute`, releaseRun).pipe();
  }

  addRelease(release: Release) {
    release.organizationId = localStorage.getItem('organizationId');
    release.projectId = localStorage.getItem('projectId');
  ...
  getReleaseRunsByFilter(releaseFilter: ReleaseFilter) {
    ...
    //return Observable.of(this.releaseRuns as ReleaseRunView[]);
  }
